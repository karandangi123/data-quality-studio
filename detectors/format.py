...
class FormatDetector(BaseDetector):
    def detect(self, conn, raw_table: str, ref_table: str) -> List[Issue]:
        ...
        if 'phone' in raw_cols:
            invalid_phones = conn.execute(rf"""
                SELECT COUNT(*) FROM {raw_table}
            ...
            if invalid_phones > 0:
                issues.append(Issue(
                    ...
                    sql_fix=rf"SELECT * REPLACE ('+91-' || RIGHT(regexp_replace(CAST(phone AS VARCHAR), '[^0-9]', '', 'g'), 10) AS phone) FROM {raw_table};"
                ))
